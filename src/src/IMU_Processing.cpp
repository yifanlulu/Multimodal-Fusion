...
void ImuProcess::UndistortPcl(LidarMeasureGroup &lidar_meas, StatesGroup &state_inout, PointCloudXYZI &pcl_out)
{
  ...
  auto v_imu = meas.imu;
  auto v_mahony = lidar_meas.mahony_data;
  ...
  Eigen::Matrix3d nowRot = state_inout.rot_end;
  Eigen::Matrix3d transRot;
  ...
  case VIO:
    ...
    for (int i = 0; i < v_imu.size() - 1; i++)
    {
      ...
      mahonyData tail_mahony = find_mahonyData(v_mahony, tail->header.stamp.toSec());

      bool fusion_en = false;
      if (!tail_mahony.empty())
      {
        fusion_en = true;
      }
      else
      ...
      if (fusion_en)
      {
        double alpha = 0.98;
        R_imu = R_imu * Exp_f;
        Eigen::Matrix3d R_mahony = tail_mahony.q.toRotationMatrix();
        R_imu = alpha * R_imu + (1 - alpha) * R_mahony;
        Eigen::JacobiSVD<Eigen::Matrix3d> svd(R_imu, Eigen::ComputeFullU | Eigen::ComputeFullV);
        R_imu = svd.matrixU() * svd.matrixV().transpose();
      }
      else
      {
        R_imu = R_imu * Exp_f;
