...
Mahony imuMahony;
deque<mahonyData> mahony_buffer;
...
void LIVMapper::readParameters(ros::NodeHandle &nh)
{
  ...
  nh.param<bool>("mahony/file_en", imuMahony.file_en, true);
  nh.param<double>("mahony/fusionFactor", imuMahony.fusionFactor, 0.0);
  nh.param<bool>("mahony/ESKF_peidict_en", imuMahony.ESKF_peidict_en, false);
...
void LIVMapper::fusionGravity()
{
  /*************Fusion Gravity************/
  Eigen::Vector3d euler_cur = RotMtoEuler(state_propagat.rot_end) * 57.295779513;
  Eigen::Vector3d imuAngle = RotMtoEuler(LidarMeasures.lidarMahony.q.toRotationMatrix()) * 57.295779513;
  double rollMid, pitchMid, yawMid;
  rollMid = (1.0 - imuMahony.fusionFactor) * euler_cur[0] + imuMahony.fusionFactor * imuAngle[0];
  pitchMid = (1.0 - imuMahony.fusionFactor) * euler_cur[1] + imuMahony.fusionFactor * imuAngle[1];
  yawMid = euler_cur[2];
  Eigen::AngleAxisd rollAngle(AngleAxisd(rollMid * M_PI / 180.0, Vector3d::UnitX()));
  Eigen::AngleAxisd pitchAngle(AngleAxisd(pitchMid * M_PI / 180.0, Vector3d::UnitY()));
  Eigen::AngleAxisd yawAngle(AngleAxisd(yawMid * M_PI / 180.0, Vector3d::UnitZ()));
  Eigen::Quaterniond quaternion;
  quaternion = yawAngle * pitchAngle * rollAngle;
  quaternion.normalize();
  state_propagat.rot_end = quaternion;
  if (1)
  { // debug info
    std::cout << "-------------------------------" << std::endl;
    std::cout << "fastlivo2 rpy = " << euler_cur[0] << " " << euler_cur[1] << " " << euler_cur[2] << std::endl;
    std::cout << "mahony rpy = " << imuAngle[0] << " " << imuAngle[1] << " " << imuAngle[2] << std::endl;
    std::cout << "fusion rpy = " << rollMid << " " << pitchMid << " " << yawMid << std::endl;
    std::cout << "delta rpy = " << euler_cur[0] - imuAngle[0] << " " << euler_cur[1] - imuAngle[1]
              << " " << euler_cur[2] - imuAngle[2] << std::endl;
    static Eigen::Vector3d maxRPY;
    Eigen::Vector3d nowdelta = euler_cur - imuAngle;
    if (abs(nowdelta[2]) > 180)
      nowdelta[2] = 360.0 - abs(nowdelta[2]);
    if (abs(nowdelta[0]) > maxRPY[0])
      maxRPY[0] = abs(nowdelta[0]);
    if (abs(nowdelta[1]) > maxRPY[1])
      maxRPY[1] = abs(nowdelta[1]);
    if (abs(nowdelta[2]) > maxRPY[2])
      maxRPY[2] = abs(nowdelta[2]);
    std::cout << "max delta rpy = " << maxRPY[0] << " " << maxRPY[1] << " " << maxRPY[2] << std::endl;
  }
}
...
void LIVMapper::save_pose_tum()
{
  if (LidarMeasures.lio_vio_flg == VIO)
  {
    return;
  }
  const std::string filename = "/home/yifan/code/TC-slam/fastlivo2_advance/src/FAST-LIVO2/Log/pose.txt";
  ...
  if (outfile.is_open())
  {
    ...
    Eigen::Quaterniond quat(state_propagat.rot_end);
    outfile << LidarMeasures.last_lio_update_time << " "
...
void LIVMapper::imu_cbk(const sensor_msgs::Imu::ConstPtr &msg_in)
{
  ...
  {
    Eigen::Quaterniond q = imuMahony.MahonyAHRSupdateIMU(msg->angular_velocity,
                                                         msg->linear_acceleration,
                                                         timestamp - last_timestamp_imu);
    mahony_buffer.push_back(mahonyData(timestamp, q));
  }
...
bool LIVMapper::sync_packages(LidarMeasureGroup &meas)
{
  ...
  case ONLY_LIO:
  {
    ...
    while (!imu_buffer.empty())
    {
      ...
      mahonyData data = find_mahonyData(mahony_buffer, imu_buffer.front()->header.stamp.toSec(), true);
      if (!data.empty())
      {
        if (imuMahony.ESKF_peidict_en)
        {
          meas.mahony_data.push_back(data);
        }
        meas.lidarMahony = data;
      }
      else
      {
        std::cout << "-------------------no find mahony_data" << std::endl;
      }
    }
